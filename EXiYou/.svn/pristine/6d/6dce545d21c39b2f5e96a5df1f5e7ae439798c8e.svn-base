...
import jcp.com.exiyou.ui.countrysite.bean.CountrySiteHotCity;
import jcp.com.exiyou.ui.countrysite.bean.CountrySiteNewDiscount;
...
public class CountryGoAdapter extends BaseListViewAdapter<CountrySiteNewDiscount> {
    private Context context;
    public CountryGoAdapter(Context context, String mType, List<CountrySiteNewDiscount> mData, int... layoutIds) {
        super(context, mType, mData, layoutIds);
        this.context=context;
    }

    @Override
    protected void loadData(ViewHolder holder, CountrySiteNewDiscount item, int position) {
        holder.setImage(context, R.id.country_go_image,item.getPhoto());
        holder.setText(R.id.country_go_title,item.getTitle());
        String price = item.getPrice();
        int i = price.indexOf(">");
        int i1 = price.lastIndexOf("<");
        String substring = price.substring(i+1, i1);
        holder.setText(R.id.country_go_money, substring);
        holder.setText(R.id.country_go_zhekou,item.getPriceoff());
    }
}
