package jcp.com.exiyou.ui.countrysite.persenter;
...
public class CountryMoreCityPresenter extends CountryMoreCityContract.CountryMoreCityPersenter {
    private static final String TAG = CountryMoreCityPresenter.class.getSimpleName();

    @Override
    public void getCountryMoreCityPage(final CountryMoreCityActivity.State state, int page, int country_id) {
        mModel.getCountryCity(state,page, country_id).subscribe(new Subscriber<CountryMoreCityBean>() {
            @Override
            ...
            @Override
            public void onNext(CountryMoreCityBean countryMoreCityBean) {
                mView.returnCity(state,countryMoreCityBean);
                Log.e(TAG, "onNext: "+countryMoreCityBean.getData().size() );
            }
        });
    }
}
