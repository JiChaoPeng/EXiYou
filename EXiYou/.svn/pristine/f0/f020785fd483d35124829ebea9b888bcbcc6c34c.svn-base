...
public class CountryPresenter extends CountryContract.CountryPersenter {
    private static final String TAG = CountryPresenter.class.getSimpleName();

    @Override
    public void CountryBean(int country_id) {
        mModel.CountryBean(country_id).subscribe(new Subscriber<CountrySiteBean>() {
            @Override
...
//                Log.e(TAG, "onError: " );
            ...
            @Override
            public void onNext(CountrySiteBean countrySiteBean) {
//                Log.e(TAG, "onNext: "+countrySiteBean.getData().getId() );
                mView.retrunList(countrySiteBean);
            }
        });
    ...
    @Override
    public void CountryPager(int wight, int hight, Context context, CountrySiteBean countrySiteBean) {
        List<ImageView> viewPager = mModel.CountryPager(wight, hight, context, countrySiteBean);
        mView.returnViewPager(viewPager);
    }
}
