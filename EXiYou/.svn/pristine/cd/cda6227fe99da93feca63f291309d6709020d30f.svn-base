...
public class CityReadMoreAdapter extends BaseListViewAdapter<CitySiteBean.DataBean.MguideListsBean> {
    private Context context;
    public CityReadMoreAdapter(Context context, String mType, List<CitySiteBean.DataBean.MguideListsBean> mData, int... layoutIds) {
        super(context, mType, mData, layoutIds);
        this.context=context;
    }

    @Override
    protected void loadData(ViewHolder holder, CitySiteBean.DataBean.MguideListsBean item, int position) {
        holder.setText(R.id.city_read_more_item_text,item.getMguide_name());
        holder.setImage(context,R.id.city_read_more_item_image,item.getMguide_photo()+"500");
    }
}
