package jcp.com.exiyou.ui.citysite.persenter;
...
public class CityPresenter extends CityContract.CityPersenter{
    private static final String TAG = CityPresenter.class.getSimpleName();

    @Override
    public void CityBean(String city_id) {
        mModel.CityBean(city_id).subscribe(new Subscriber<CitySiteBean>() {
            @Override
...
//                Log.e(TAG, "onError: " );
            ...
            @Override
            public void onNext(CitySiteBean citySiteBean) {
//                Log.e(TAG, "onNext: "+citySiteBean.getData().getId() );
                mView.retrunList(citySiteBean);
            }
        });
    ...
    @Override
        public void CityPager(int wight, int hight, Context context, CitySiteBean citySiteBean) {
        List<ImageView> viewPager = mModel.CityPager(wight, hight, context, citySiteBean);
