package jcp.com.exiyou.ui.citysite.model;
...
public class CityModel implements CityContract.CityModel {
    @Override
    public Observable<CitySiteBean> CityBean(String city_id) {

        return Api.getHostService().getCityRes(city_id).compose(new
                AndroidIOToMain.IOToMainTransformer<CitySiteBean>());
    }

    @Override
    public List<ImageView> CityPager(int wight, int hight, Context context, CitySiteBean citySiteBean) {
        List<ImageView> list = new ArrayList<>();
        int size = citySiteBean.getData().getCity_pic().size();
        if (size == 0) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                Picasso.with(context).load(citySiteBean.getData().getCity_pic().get(i)).into(imageView);
                list.add(imageView);
            }
        }
