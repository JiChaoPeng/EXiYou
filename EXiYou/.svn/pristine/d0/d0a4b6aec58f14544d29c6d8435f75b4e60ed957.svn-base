...
public class CountryMoreCityModel implements CountryMoreCityContract.CountryMoreCityModel {
    @Override
    public Observable<CountryMoreCityBean> getCountryCity(CountryMoreCityActivity.State state,int page, int country_id) {
        return Api.getHostService().getMoreCityRes(page,country_id).compose(new
                AndroidIOToMain.IOToMainTransformer<CountryMoreCityBean>());
    }
}
