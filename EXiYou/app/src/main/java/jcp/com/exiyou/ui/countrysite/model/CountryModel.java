package jcp.com.exiyou.ui.countrysite.model;
...
public class CountryModel implements CountryContract.CountryModel {
    @Override
    public Observable<CountrySiteBean> CountryBean(int country_id) {

        return Api.getHostService().getCountryRes(country_id).compose(new
                AndroidIOToMain.IOToMainTransformer<CountrySiteBean>());
    }

    @Override
    public List<ImageView> CountryPager(int wight, int hight, Context context, CountrySiteBean countrySiteBean) {
        List<ImageView> list = new ArrayList<>();
        int size = countrySiteBean.getData().getPhotos().size();
        if (size == 0) {
            for (int i = 0; i < size; i++) {
                ImageView imageView = new ImageView(context);
                imageView.setImageResource(R.mipmap.bg_hotel_main);
                imageView.setScaleType(ImageView.ScaleType.CENTER_CROP);
                list.add(imageView);
                ImageView imageView1= new ImageView(context);
                imageView1.setImageResource(R.mipmap.dest_tab_worldmap);
            ...
            for (int i = 0; i < size; i++) {
                ImageView imageView = new ImageView(context);
                imageView.setScaleType(ImageView.ScaleType.CENTER_CROP);
                LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(wight, hight);
                imageView.setLayoutParams(layoutParams);
                Picasso.with(context).load(countrySiteBean.getData().getPhotos().get(i)).into(imageView);
                list.add(imageView);
            }
        }
