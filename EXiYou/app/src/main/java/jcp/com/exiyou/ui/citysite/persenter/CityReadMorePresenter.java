...
public class CityReadMorePresenter extends CityReadMoreContract.CityReadMorePersenter {
    @Override
    public void CityReadMorePageBean(final CityReadMoreActivity.State state, String id, int page) {
        mModel.getListBean(state,id,page).subscribe(new Subscriber<CityReadMoreBean>() {
            @Override
            ...
            @Override
            public void onNext(CityReadMoreBean cityReadMoreBean) {
                mView.retrunList(state,cityReadMoreBean);
            }
        });
    }
}
