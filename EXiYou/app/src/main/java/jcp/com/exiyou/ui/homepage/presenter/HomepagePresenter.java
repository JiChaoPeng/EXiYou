package jcp.com.exiyou.ui.homepage.presenter;
...
public class HomepagePresenter extends HomePageContract.HomePagePersenter {
    private static final String TAG = HomepagePresenter.class.getSimpleName();
    @Override
    public void getHomePageBean(final HomePageFragment.State state, int page) {
        mModel.getListBean(state,page).subscribe(new Subscriber<HomePageBean>() {
            @Override
            ...
            public void onNext(HomePageBean homePageBean) {
                mView.retrunList(state,homePageBean);//数据返回的地方，通过View传回给Activity
            }
        });
    }
    @Override
    public void getViewPager( int wight,int hight, Context context,HomePageBean homePageBean) {
        List<ImageView> viewPager = mModel.getViewPager(wight, hight, context, homePageBean);
