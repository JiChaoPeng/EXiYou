...
//        Log.e(TAG, "retrunList: " + homePageBean.getDataBean().getFeed().getEntry().size());
    ...
    private void addRadioButton() {//添加四个小圆点 最新的可以使用Banner图进行轮播页的添加
    ...
    @Override
    public boolean handleMessage(Message message) {//发送轮播页的延迟两秒 并不停地进行延迟发送
        switch (message.what) {
            case VIEWPAGER_H:
...
//            Log.e(TAG, "onPageScrollStateChanged: "+position );
    ...
    public void onResume() {
        ...
        /**
         * sharedPreferences进行存储数据 首页的四个最近地点
         */
