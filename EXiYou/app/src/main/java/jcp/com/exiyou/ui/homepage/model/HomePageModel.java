package jcp.com.exiyou.ui.homepage.model;
...
public class HomePageModel implements HomePageContract.HomePageModel {
    ...
    @Override
    public Observable<HomePageBean> getListBean(HomePageFragment.State state, int page) {
        //通过枚举确认是上拉刷新还是上拉加载。通过网络地址拼接的page进行网络请求
    return Api.getHostService().getHomePageRes(page).compose(new
                                                             AndroidIOToMain.IOToMainTransformer<HomePageBean>());
}
    //首页轮播的数据 返回的是ImageView的集合
    @Override
    public List<ImageView> getViewPager( int wight,int hight, Context context, HomePageBean homePageBean) {
        ...
        for (int i = 0; i <size; i++) {
            ...
            Picasso.with(context).load(homePageBean.getDataBean().getSlide().get(i).getPhoto()).into(imageView);
            list.add(imageView);
        }
        return list;
    }
