...
public class SplashActivity extends AppCompatActivity implements Animator.AnimatorListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        setTheme(R.style.AppTheme);//首次进入不会出现白屏,进入出现一个主题 主题图片与一开始进入的图片相同
    ...
    private void isFrist(Intent tomine, Intent towel) {
        boolean isflag=mSharedPreferences.getBoolean("frist", true);//判断是否第一次进入
        if (isflag) {
            startActivity(towel);
        }else {
    ...
    @Override
    public void onAnimationEnd(Animator animator) {//动画结束的时候进行页面跳转
